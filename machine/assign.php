...
<head>
    <meta charset="UTF-8">
    <title>Assign & Provision Users</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-900 text-white">
    <div class="container mx-auto p-8 max-w-2xl">
        <h1 class="text-3xl font-bold mb-2">Assign & Provision Users</h1>
        ...
        <form action="actions.php" method="POST" class="bg-gray-800 p-8 rounded-lg">
            <input type="hidden" name="action" value="provision_users">
            <input type="hidden" name="machine_id" value="<?= $machine['id'] ?>">

            <div class="border border-yellow-700 rounded-lg p-4 mb-8">
                <h3 class="text-xl font-semibold text-yellow-400 mb-2">Provisioning Credentials</h3>
                <p class="text-sm text-gray-400 mb-4">Provide sudo credentials for the remote machine to create users and set up SSH keys. <strong class="text-yellow-500">These are used once and are NOT stored.</strong></p>
                <div class="grid grid-cols-2 gap-4">
                    <input type="text" name="sudo_user" placeholder="Sudo Username" class="bg-gray-700 p-2 rounded-lg" required>
                    <input type="password" name="sudo_pass" placeholder="Sudo Password" class="bg-gray-700 p-2 rounded-lg" required>
                </div>
            </div>
            ...
            <div class="space-y-4 max-h-96 overflow-y-auto pr-2" id="user-list">
                ...
                <?php foreach ($all_users as $user): ?>
                    <label class="user-item flex items-center p-3 bg-gray-700 rounded-lg hover:bg-gray-600 transition cursor-pointer">
                        <input type="checkbox" name="user_ids[]" value="<?= $user['id'] ?>" class="h-5 w-5 rounded ..." <?= in_array($user['id'], $assigned_user_ids) ? 'checked' : '' ?>>
            ...
            <button type="submit" class="mt-8 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg">
                Assign & Provision Selected Users
            </button>
        </form>
